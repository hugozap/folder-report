...
function generateReport(folders){
	...
	var indexTemplatePath = path.join(__dirname,templatePath);
	console.log('path template:'+indexTemplatePath);
	var templateContents = fs.readFileSync(indexTemplatePath);
	var template = Hogan.compile(templateContents);
	var result = template.render(folders);
